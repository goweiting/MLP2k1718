...
for trial in range(1,4):
    for learningRule, lrfunc in learningRules.items():
        train_data.reset()
        test_data.reset()
        valid_data.reset()
        ...
        learning_rule = lrfunc

        experiment_units_elu[learningRule] = train_model_and_plot_stats(
            model,
            error,
            learning_rule,
            train_data,
    ...
    #===================== pickle: =====================================#
    pkl.dump(experiment_units_elu, open('LR_{}.pkl'.format(trial),'wb'), protocol=-1)
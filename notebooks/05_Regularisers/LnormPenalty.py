...
weight_penalties= {
"L1(1e-5)" : L1Penalty(1e-5),
"L1(1e-3)" : L1Penalty(1e-3),
"L2(1e-4)" : L2Penalty(1e-4),
"L2(1e-2)" : L2Penalty(1e-2)
}


for trial in range(1,4):
    for label, weight_penalty in weight_penalties.items():
        train_data.reset()
        test_data.reset()
        valid_data.reset()
        ...
        input_layer = [
            AffineLayer(input_dim, hidden_dim, weights_init, biases_init, weights_penalty=weight_penalty)
        ]
        output_layer = [
            func,
            AffineLayer(hidden_dim, output_dim, weights_init, biases_init, weights_penalty=weight_penalty)
        ]
        each_hidden_layer = [
            func,
            AffineLayer(hidden_dim, hidden_dim, weights_init, biases_init, weights_penalty=weight_penalty)
        ]
        ...
        experiment_units_elu[label] = train_model_and_plot_stats(
            model,
            error,
            learning_rule,
            train_data,
    ...
    #===================== pickle: =====================================#
    pkl.dump(experiment_units_elu, open('LNorm12_{}.pkl'.format(trial),'wb'), protocol=-1)
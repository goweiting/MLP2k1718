...
def train_model_and_plot_stats(model,
    ...
    if earlyStopping:
        ...
        return (optimiser, stats, key, run_time, best_epoch)
    else:
        optimiser = Optimiser(
            model,
            error,
...
optimiser, stats, key, run_time = train_model_and_plot_stats(
    model,
    error,
    learning_rule,
    train_data,
    ...
    notebook=False,
    earlyStopping=False)
_model = optimiser.model
try:
    pkl.dump((stats, key, run_time), open('BASELINE_2layer_4DNN.{}.pkl'.format(trial), 'wb'), protocol=-1)
except:
    print("STATS\n",run_time)
    print("STATS\n",stats)
    print("KEYS\n", key)
try:
    pkl.dump(_model, open('BASELINE_2layer_4DNN.{}.model.pkl'.format(trial), 'wb'), portocol=-1)
except:
    print(_model)